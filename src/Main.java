...
public class Main {
	...
	private static double[][] unknown = new double[784][10];
	...
	public static void main(String[] args) {
		...
		//read unknown.txt and put into 2-dim arrat unknown
		readUnkown();
		...
		//run the unknown inputs through the trained neural network
		run_unknown(n1);
	...
	public static void run_unknown(NeuronNetwork n)
	{
		...
		//for all inputlines of unknown.txt
		for(int k=0;k<784;k++)
		{
			//read 10 new values as inputs
			for(int m=0;m<10;m++)
			{
				testinputs[m]=unknown[k][m];
			}

			//calculate the outputs of unknown.txt with the trained neural network
			testoutputs = n.calculate_Outputs(testinputs, hiddenlayers_amount+1);
		...
		//put outputs into txt file
		classes_to_txtfile(output_targets);
	...
	public static void classes_to_txtfile(double[] data)
	{
		//Prints the data to textfile 19_classes.txt
		PrintWriter writer;
		try {
			writer = new PrintWriter("src\\19_classes.txt", "UTF-8");
			for(int i=0; i<data.length;i++)
			{
				writer.print((int)data[i]);
				writer.print(", ");
			}
			writer.close();
	...
	public static void readUnkown(){
		Scanner sc = null;
		try {
			sc = new Scanner(new File("src\\unknown.txt"));
		} catch (FileNotFoundException e) {
			// TODO Auto-generated catch block
			e.printStackTrace();
		}
		...
		for(int i=0; i<783; i++){
			...
			String[] values = line.split(",");
			for (int j=0; j<10; j++){
				unknown[i][j] = Double.valueOf(values[j]);
			}
		}
